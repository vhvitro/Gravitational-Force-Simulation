...
def consume_body(consumer, consumed):
    ...
    if consumed.mass < constants['collision_threshold']:
        print("Collision led to total comsunption")
        consumer.mass += consumed.mass
    else:
        print(f"Collision in pos {(consumer.x, consumer.y)} not total")    
...
def handle_collision(body1, body2):
    ...
    if body1.mass >= MASS_RATIO_THRESHOLD * body2.mass or body2.mass >= body1.mass * MASS_RATIO_THRESHOLD:
        e = 0  # Perfectly inelastic

        # Handle merging/absorbtion
        if body2.mass >= BLACK_HOLE_MASS and body1.mass >= BLACK_HOLE_MASS:
            ...
            print(f"Merged black holes - pos:{(merged_x, merged_y)}")
            ...
            if body2.mass >= constants['collision_threshold']:
                ...
                # Create debris
                escape_velocity = math.sqrt(G * body1.mass / ((body1.radius + body2.radius)/SCALE))
            ...
            if body1.mass >= constants['collision_threshold']:
                ...
                escape_velocity = math.sqrt(G * body2.mass / ((body1.radius + body2.radius)/SCALE))
